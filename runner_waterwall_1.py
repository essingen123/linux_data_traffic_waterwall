...
def block_process(pid):
    subprocess.call(f"sudo iptables -A OUTPUT -m owner --uid-owner {pid} -j DROP", shell=True)

# Unblock traffic for a specific process
def unblock_process(pid):
    subprocess.call(f"sudo iptables -D OUTPUT -m owner --uid-owner {pid} -j DROP", shell=True)
...
def set_traffic_limit(pid, percentage):
    ...
    subprocess.call(f"sudo iptables -A OUTPUT -m owner --uid-owner {pid} -m limit --limit-burst {bandwidth_limit} -j ACCEPT", shell=True)
